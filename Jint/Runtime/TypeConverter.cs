...
namespace Jint.Runtime
{
    ...
    public class TypeConverter
    {
        ...
        public static void CheckObjectCoercible(Engine engine, JsValue o, MemberExpression expression, object baseReference)
		{
			...
			throw new JavaScriptException(engine.TypeError, message) {Location = expression.Location};
		}
            ...
            methods = methods.Where(m =>
                {
                    ...
                    var min = parameters.Count() - parameters.Count(p => (p.Attributes & ParameterAttributes.HasDefault) == ParameterAttributes.HasDefault);
                    return parameters.Count() >= arguments.Length && min <= arguments.Length;
                })
                .ToArray();
